...
export function 批量插入空行() {
    ...
    for (let i = 0; i < 行列表.length; i++) {
        ...
        // 检查是否为特殊行（表格、引用）
        const 是当前特殊行 = 是特殊行(当前行);
        const 是上一行特殊行 = i > 0 ? 是特殊行(行列表[i - 1]) : false;
        
        // 列表行也跳过
        const 是列表行 = /^\s*(\-|\+|\*|\d+\.)\s/.test(当前行);
        const 是上一行列表行 = i > 0 ? /^\s*(\-|\+|\*|\d+\.)\s/.test(行列表[i - 1]) : false;
        
        // 空行跳过
        const 是空行 = /^\s*$/.test(当前行);
        const 是上一行空行 = i > 0 ? /^\s*$/.test(行列表[i - 1]) : false;
        const 上一行是代码块边界 = i > 0 ? /^\s*```/.test(行列表[i - 1]) : false;
        
        // 决定是否在当前行前插入空行
        // 条件：前一行非空、前一行不是特殊行、当前行不是特殊行、前后都不是列表行、前一行不是空行、前一行不是代码块边界
        if (i > 0 && 
            !是上一行空行 && 
            !是空行 &&
            !是上一行特殊行 && 
...
export function 批量去除空行() {
    ...
    for (let i = 0; i < 行列表.length; i++) {
        ...
        // 判断前一行和后一行是否为特殊行
        const 是上一行特殊行 = i > 0 ? 是特殊行(行列表[i - 1]) : false;
        const 是下一行特殊行 = i < 行列表.length - 1 ? 是特殊行(行列表[i + 1]) : false;
        const 是上一行引用 = i > 0 ? /^\s*>/.test(行列表[i - 1]) : false;
        const 是下一行引用 = i < 行列表.length - 1 ? /^\s*>/.test(行列表[i + 1]) : false;
        
        // 如果是空行且前后都不是特殊行（表格、引用），则跳过该空行
        if (是空行 && !是上一行特殊行 && !是下一行特殊行 && !是上一行引用 && !是下一行引用) {
            continue;
        }
